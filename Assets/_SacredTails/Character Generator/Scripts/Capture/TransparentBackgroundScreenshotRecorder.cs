...
public class TransparentBackgroundScreenshotRecorder : MonoBehaviour
{
    ...
    #endregion
    public static System.Action OnTakePhoto;
    void Awake()
    {
        if (!ColorSwapper.Instance._dontTakePhoto)
        {
    ...
    //     else
    //     {
    //         SacredTailsLog.LogMessage("Complete! " + videoFrame + " videoframes rendered. File names are 0 indexed)");
    //         Debug.Break();
    //     }
    // }
    ...
    IEnumerator CaptureFrame()
    {
        ...
        if (videoFrame < framesToCapture)
        {
            ...
            SavePng();
            videoFrame++;
            //SacredTailsLog.LogMessage("Rendered frame " + videoFrame);
            videoFrame++;
        }
        else
        {
