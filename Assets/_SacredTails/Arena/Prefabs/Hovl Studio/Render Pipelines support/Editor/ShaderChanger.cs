...
public class RPChanger : EditorWindow
{
    ...
    private void ImportPipelinePackage()
    {
        ...
        switch (pipeline)
        {
            ...
            case 2:
                ...
                break;
            default:
                SacredTailsLog.LogMessage("You didn't choose pipeline");
                break;
        }
#endif
    ...
    {
        //SacredTailsLog.LogMessage($"Imported package: {packagename}");
        FindShaders();
        switch (pipeline)
        {
            case 1:
            ...
            if (Shader.Find("Shader Graphs/URP_Add_CG") != null)
            {
                if (material.shader == Add_CG || material.shader == Add_CG_HDRP)
                {
                        ...
                        || material.GetTexture("_Flow") != null || material.GetTexture("_Mask") != null)
                    {
                        ...
                        material.shader = Add_CG_URP;
                    SacredTailsLog.LogMessage("Shaders changed successfully");
                }
            }
            else SacredTailsLog.LogMessage("First import shaders!");
            /*----------------------------------------------------------------------------------------------------*/
