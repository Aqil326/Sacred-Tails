...
namespace MetaMask.Transports.Unity.UI
{
    ...
    public class MetaMaskUnityUIHandler : MonoBehaviour, IMetaMaskUnityTransportListener
    {
        ...
        private void Start()
        {
            GameObject auxObjParent = GameObject.Find("LoginView");

            if(auxObjParent != null)
            {
                this.transform.parent = auxObjParent.transform;
            }

            /*if(auxParent != null)
            {
                this.transform.parent = auxParent;
            }*/
        }
